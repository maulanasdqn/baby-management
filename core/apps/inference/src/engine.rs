...
impl Engine {
    ...
    fn load_weights(cfg: &GemmaConfig, path: &str, device: &NdArrayDevice) -> Result<GemmaModel<B>, String> {
        ...
        // Weight loading: in production, iterate st.tensors() and assign to model params
        // This scaffold builds and checks the architecture; weight injection is wired in build.sh
        let _ = st.names(); // Ensure file is readable
        Ok(model)
    }
    ...
    pub fn generate(
        ...
        while generated < max_tokens {
            ...
            // Greedy decode: pick argmax of last token
