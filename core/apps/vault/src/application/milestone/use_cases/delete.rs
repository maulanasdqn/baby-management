...
mod tests {
    ...
    #[test]
    fn delete_existing_milestone_succeeds() {
        let id = Uuid::new_v4();
        ...
        let uc = DeleteMilestoneUseCase::new(&repo);
        assert!(uc.execute(id).is_ok());
        assert!(repo.0.lock().unwrap().is_empty());
    }

    #[test]
    fn delete_nonexistent_milestone_returns_not_found() {
